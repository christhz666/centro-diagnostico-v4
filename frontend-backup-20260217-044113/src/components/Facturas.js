...
const Facturas = () => {
  ...
  const fetchCitasPendientes = async () => {
    try {
      ...
      setCitasPendientes((response.data || []).filter(c => c.estado === 'completada' || c.estado === 'programada'));
    } catch (err) {
      console.error(err);
      setCitasPendientes([]);
    }
  ...
  const verDetalle = async (factura) => {
    try {
      const response = await api.getFactura(factura._id);
      setFacturaDetalle(response.data || response);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const imprimirFactura = async (factura) => {
    try {
      const response = await api.getFactura(factura._id);
      const facturaCompleta = response.data || response;
      ...
      const estudios = (facturaCompleta.items || []).map(item => ({
        nombre: item.descripcion || item.nombre || 'Estudio',
        precio: item.precioUnitario || item.precio || item.subtotal || 0,
  ...
  const calcularTotalHoy = () => {
    ...
    return facturas
      .filter(f => new Date(f.createdAt).toDateString() === hoy && f.estado !== 'anulada')
            ...
            {facturas.length === 0 ? (
              ...
              facturas.map(f => (
                <tr key={f._id} style={{ borderBottom: '1px solid #eee' }}>
                  <td style={{ padding: 15, fontFamily: 'monospace' }}>{f.numero}</td>
                  <td style={{ padding: 15 }}>{new Date(f.createdAt).toLocaleDateString('es-DO')}</td>
                  <td style={{ padding: 15 }}>{getTexto(f.datosCliente?.nombre || f.paciente?.nombre)}</td>
      ...
      {facturaDetalle && (
        <div style={{
          ...
          <div style={{ background: 'white', padding: 30, borderRadius: 15, width: '100%', maxWidth: 600, maxHeight: '90vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h2 style={{ margin: 0 }}>Factura {facturaDetalle.numero}</h2>
            ...
            <div style={{ background: '#f8f9fa', padding: 15, borderRadius: 8, marginBottom: 20 }}>
              <p style={{ margin: '5px 0' }}><strong>Cliente:</strong> {getTexto(facturaDetalle.datosCliente?.nombre || facturaDetalle.paciente?.nombre)}</p>
              <p style={{ margin: '5px 0' }}><strong>Cedula:</strong> {getTexto(facturaDetalle.datosCliente?.cedula || facturaDetalle.paciente?.cedula)}</p>
              <p style={{ margin: '5px 0' }}><strong>Fecha:</strong> {new Date(facturaDetalle.createdAt).toLocaleDateString('es-DO')}</p>
            </div>
