...
const FacturaTermica = ({ factura, paciente, estudios, onClose }) => {
  ...
  const subtotal = estudios.reduce((sum, e) => sum + (e.precio || e.precioUnitario || 0), 0);
  const cobertura = estudios.reduce((sum, e) => sum + (e.cobertura || 0), 0);
  ...
  const codigoPaciente = 'PAC' + (paciente?._id || paciente?.id || '000000').slice(-8).toUpperCase();
  const numeroFactura = factura?.numero || 'F-' + Date.now().toString().slice(-8);
      ...
      <div className="factura-termica">
        ...
        <div style={{ borderTop: '2px solid ' + colores.azulOscuro, borderBottom: '2px solid ' + colores.azulOscuro, padding: '8px 0', marginBottom: '10px' }}>
          <p style={{ margin: '3px 0', fontSize: '11px' }}>
            <strong>Paciente:</strong> {getTexto(paciente?.nombre)} {getTexto(paciente?.apellido)}
          </p>
          <p style={{ margin: '3px 0', fontSize: '11px' }}>
