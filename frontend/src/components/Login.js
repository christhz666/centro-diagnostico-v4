    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('/api/auth/login', {
                ...
                body: JSON.stringify({ email, password })  // ? CORREGIDO: era "username"
            });

            const data = await response.json();

            // ? CORREGIDO: Flask devuelve access_token, no token
            ...
            <div style={styles.card}>
                ...
                <form onSubmit={handleSubmit} style={styles.form}>
                    ...
                    <button type="submit" style={{...styles.button, opacity: loading ? 0.7 : 1}} disabled={loading}>
                        {loading ? <><FaSpinner style={{marginRight: 8, animation: 'spin 1s linear infinite'}} /> Iniciando...</> 
                                 : <><FaSignInAlt style={{marginRight: 8}} /> Iniciar Sesión</>}
                    </button>
                </form>

                <div style={styles.demoInfo}>
                    <p style={{margin: 0, fontSize: '13px', fontWeight: 'bold'}}>Accesos de prueba:</p>
                    <p style={{margin: '5px 0 0', fontSize: '12px', fontFamily: 'monospace'}}>admin@miesperanza.com / Admin123!</p>
                </div>
            </div>
        </div>
    );
